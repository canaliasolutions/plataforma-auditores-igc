...
import {Auditoria, ResumenAuditoria} from "@/schemas/types";
...
export async function getAudits(auditorEmail: string): Promise<ResumenAuditoria[]> {
    ...
    auditsRawData.forEach((auditRawItem: {[key: string]: string}) => {
        ...
        auditoria.nombre_cliente = auditRawItem['Clientes 2::clienteNombre'];
...
export async function getAuditById(auditId: string, auditorEmail: string): Promise<Auditoria | null> {
    ...
    try {
        ...
        console.warn("Error obteniendo auditoria de filemaker: ", e);
        throw new Error(`Error obteniendo la auditoria ${auditId} de filemaker: ${e}`);
    }
    ...
    return {
        ...
        nombre_cliente: auditRawItem['Clientes 2::clienteNombre'],
